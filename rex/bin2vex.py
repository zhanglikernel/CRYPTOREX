...
def extractfile(filename,outputdir):
	...
	global numfirmware;
	numfirmware += 1;
	for module in binwalk.scan(filename,"-r","-e" ,"-y", "filesystem", signature=True, quiet=True, extract=True):
...
def listallso(filename,arch):
	...
	(status,output) = commands.getstatusoutput( commandstr + ' ' + filename);
	#print(filename);
	#print(filename);
...
def _getarch(filename):
	...
	if not any(s in [filetype] for s in [b"application/x-executable",b"application/x-sharedlib"]):
		return result;
	(status,output) = commands.getstatusoutput('readelf -h ' + filename);
	outstr = output.splitlines();
	if not len(outstr) > 9:
		return result;
...
def isusecrypt(filename,arch):
	...
	if arch == archtype.Elarm:
		(status,output) = commands.getstatusoutput('armobjdump.sh ' + filename + " | grep -E -i \"gcry|EVP|CAST|SSL|encry|cry|aes|DES|decry\"");
		if(len(output) > 0):
			outstr = output.splitlines();
			outputline = filename;
			for line in outstr:
		...
		(status,output) = commands.getstatusoutput('mipsobjdump.sh ' + filename + " | grep -E -i \"gcry|EVP|CAST|SSL|encry|cry|aes|DES|decry\"");
		if(len(output) > 0):
                        outstr = output.splitlines();
                        outputline = filename;
                        for line in outstr:
...
def getfileowner(filename):
	filename = filenameformat(filename);
	(status,output) = commands.getstatusoutput('ls -al ' + filename);
	print(filename);
	print(output);
