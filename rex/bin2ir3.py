...
def constructmycfg(node,proj,functionmap,lcom,mycfg,addrset,pathnode,test,bpspoffset):
...
def reversetracedfs(nodeset,tinfo,offset,alldata,soffset,depth,execfuncdetail,allfn,middlefunc,ef,detailreport,wfuncsetreturn,pathroad):
	node = nodeset[0];
	irsb = node.getirsb();
	takeaction = False;
	actionv = list();
	...
	for pitem in node.getparents():
		reversetracedfs(pitem,traceinfocopy(tinfo),offset,alldata,node.stackoffset,depth+1,execfuncdetail,allfn,middlefunc,ef,detailreport,wfuncsetreturn,copy.copy(pathroad));

def reversetrans(nodeset,function,offset,alldata,execfuncdetail,allfuncn,ef,detailreport,wfuncsetreturn):
	node = nodeset[0];
	ti = func2traceinfo(function,node.beginaddr);
	middlefunc = dict();
	reversetracedfs(nodeset,ti,offset,alldata,0,1,execfuncdetail,allfuncn,middlefunc,ef,detailreport,wfuncsetreturn,set());
	return middlefunc;
...
def transbin2IR(filename,outputname,importfc = dict()):
	...
	funcnameset = funcnameset.union(importfc.keys())
	...
	for item in funcdic:
		...
		starttime = time.time();
		node,leafset = constructmycfg(node,proj,functionmap,lcom,mycfg,alladdrset,copy.copy(set()),0,None);
	...
	for item in externfunctiondic:
		for elitem in externfunctiondic[item]:
			...
			for pitem in elitem.getparents():
				...
				#global starttime;
				starttime = time.time();
				tmpresult = reversetrans(pitem,function,offset,alldata,execfuncdetail,allfuncname,exportedfunc,detailreport,wfuncsetreturn);
